...
class FileNode:
    def __init__(self,name,isFile,path,Qr,Qw,meta_server,data_servers):
        self.name = name
        self.path = path
        self.Qr = Qr
        self.Qw = Qw
    ...
    def put(self,key,value):
        ...
        if key[-4:] == "meta" or key[-10:] == "list_nodes" :
                ...
                while (putmetastatus is False):               
                        try:
                                putmetastatus = rpc.put(Binary(key),Binary(pickle.dumps(value)),6000)
                ...
                for number in data_servers:
                        ...
                        while (putdatastatus is False and putdatacount < 5):               
                                try:
                                        putdatastatus = rpc.put(Binary(key),Binary(pickle.dumps(value)),6000)
                                        #print "Write is successful at",number
                                except:
        ...
        if key[-4:] == "meta" or key[-10:] == "list_nodes" :
                ...
                if highest > int(Qr) :
                        for data,count in list_map.items() :
                                if count == highest :
                                        correct_value = data
                                        print "correct value is : ", pickle.loads(correct_value)
                                break                        
                        for number in data_servers:
                                port = 'http://127.0.0.1:'+ str(number)
                                rpc = xmlrpclib.Server(port)
                                ...
                                if correct_value != c[k] :
                                        ...
                                        while (status is False and counter < 5) :
                                                try :
                                                        status = rpc.put(Binary(key),Binary(pickle.dumps(correct_value)),6000)
                                                except :
...
class FS:
    def __init__(self,Qr,Qw,meta_server,data_servers):
        self.Qr = Qr
        self.Qw = Qw
        self.meta_server = meta_server
        self.data_servers = data_servers
        self.root = FileNode('/',False,'/',Qr,Qw,meta_server,data_servers)
        now = time()
        self.fd = 0
    ...
    def add_dir(self,path,mode):
        # create a file node
        temp_node = FileNode(path.split('/')[-1],False,path,Qr,Qw,meta_server,data_servers)
    ...
    def add_file(self,path,mode):
        # create a file node
        temp_node = FileNode(path.split('/')[-1],True,path,Qr,Qw,meta_server,data_servers)
    ...
    def link_nodes(self,target,source):
        ...
        temp_node = FileNode(target.split('/')[-1],True,target,Qr,Qw,meta_server,data_servers)
...
class Memory(LoggingMixIn, Operations):
    ...
    def __init__(self,Qr,Qw,meta_server,data_servers):
        ...
        self.FS = FS(Qr,Qw,meta_server,data_servers)
...
if __name__ == "__main__":
  ...
  fuse = FUSE(Memory(Qr,Qw,meta_server,data_servers), argv[1], foreground=True)
