...
class FileNode:
    def __init__(self,name,isFile,path,url):
        ...
        db = mongoc.fsdb
        fsys = db.fs 
        key = self.path+"&&"+"meta" # true if node is a file, false if is a directory.
        if not fsys.find_one({"key":key}):
    ...
    def put(self,key,value):
        ...
        fdict = dict()
        fdict = {"key":key, "value":value}
        
        print "value", value
        print "\n \n put fdict \n\n", fdict
                          
        self.id = fsys.update({"key":key},{ "$set": { "value": pickle.dumps(value) } },upsert =True)
    ...
    def get(self,key):
        ...
        #print "root fs: ", fsys.find_one({"key":'/&&meta'})
        
        skey = dict()
        skey = {"key":key}
        print "\n \n skey \n \n", skey
        
        fdict = fsys.find_one(skey)
...
class FS:
    def __init__(self,url):
        ...
        ##
        mongoc = MongoClient('127.0.0.1',27017)
        db = mongoc.fsdatabase
        #mongoc.drop_database("fsdb")
        db = mongoc.fsdb
        fsys = db.fs
        self.root = FileNode('/',False,'/',url)
        self.fd = 0
        if not fsys.find_one({"key":'/&&meta'}):
