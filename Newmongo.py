...
class FileNode:
    def __init__(self,name,isFile,path,url):
        ...
        Connectionmongo = MongoClient('mongodb://127.0.0.1:27017/')#mongo client is created
        db = Connectionmongo.testingdatabase
        fs = db.testingcollection 
        key = self.path+"&&"+"meta" 
        if not fs.find_one({"key":key}):
    ...
    def put(self,key,value):
        ...
        db = Connectionmongo.testingdatabase
        fs = db.testingcollection 
        fsindex ={"key":key, "value":value}
        self.id = fs.update({"key":key},{ "$set": { "value": pickle.dumps(value) } },upsert =True)
    ...
    def get(self,key):
        ...
        searchingk = {"key":key}
        print "\n  skey", searchingk
        fsindex = fs.find_one(searchingk)
        if "value" in fsindex.keys():
              return pickle.loads(fdict["value"])
        else:
              return None
